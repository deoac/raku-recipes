...
#         FILE: <ps-check.raku>
#
#  DESCRIPTION: Raku Recipes, Chapter 2
#               Recipe 2-3. Connect Input and Output of
#               External Utilities and Files
#
#        NOTES:
#       AUTHOR: <Shimon Bollinger>  (<deoac.bollinger@gmail.com>)
#      VERSION: 0.0.1
#     REVISION: Last modified: Wed 02 Nov 2022 04:13 PM EDT
#===============================================================================
use v6.d;
...
my Str $logfile = '/tmp/ps.log/';
my Proc::Async $proc = Proc::Async.new: 'tail', '-f', $logfile;
    ...
    with $/ {
        for .list -> $match {
            ...
            given $command {
                ...
                when none (<watch ps tail>) {
                    say "Seen $command.";
                }
            } # end given $command
        } # end for .list -> $match
    } # end with $/
} # end $proc.stdout.tap -> $v

    say "Listening to $logfile.";
    await $proc.start;
    say "Finished!";
