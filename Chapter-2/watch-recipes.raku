...
#         FILE: <.raku>
#
#  DESCRIPTION: Raku Recipes, Chapter 2.5
...
say $dir;
say $dir-watch-supply;

$dir-watch-supply.tap: -> $change {
    say $change.event;
    given $change.event {
        when FileChanged { say "{$change.path} has changed" }
        when FileRenamed { say "{$change.path} has
            moved, renamed, deleted, or created." }
    }
}

await Promise.in(300).then: { say "Finished watch!"; }
